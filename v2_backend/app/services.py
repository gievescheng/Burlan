...
def _build_document_layout_meta(document: dict | object) -> dict:
    if isinstance(document, dict):
        parser_name = str(document.get("parser_name") or "")
        parser_note = str(document.get("parser_note") or "")
        layout_page_count = int(document.get("layout_page_count") or 0)
        layout_element_count = int(document.get("layout_element_count") or 0)
    else:
        parser_name = str(getattr(document, "parser_name", "") or "")
        parser_note = str(getattr(document, "parser_note", "") or "")
        layout_page_count = int(getattr(document, "layout_page_count", 0) or 0)
        layout_element_count = int(getattr(document, "layout_element_count", 0) or 0)
    return {
        "parser_name": parser_name,
        "parser_note": parser_note,
        "layout_available": bool(layout_page_count or layout_element_count),
    ...
    for path_str in request.paths:
        try:
            ...
            document = repositories.replace_document(
                ...
                parser_note=parsed.get("parser_note", ""),
                layout_json=parsed.get("layout_json", ""),
                layout_page_count=parsed.get("layout_page_count", 0),
                layout_element_count=parsed.get("layout_element_count", 0),
            )
            layout_meta = _build_document_layout_meta(document)
            results.append(
                {
...
def audit_document(session, request) -> dict:
    ...
    if audit_cache_entry:
        ...
        for key, value in _build_document_layout_meta(document).items():
    ...
    response_data = {
        ...
        "cache_hit": False,
        **_build_document_layout_meta(document),
    }
