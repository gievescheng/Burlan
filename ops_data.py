...
# 歷史假資料已移到前端備援顯示；後端正式以原始文件與私有紀錄為主。
KIND_META = {
    ...
    "calibration": {"file": "calibration_records.json", "defaults": [], "allowed": set()},
    "environment": {"file": "environment_records.json", "defaults": [], "allowed": {".xlsx", ".csv"}},
    "training": {"file": "training_records.json", "defaults": DEFAULT_TRAINING, "allowed": set()},
    "equipment": {"file": "equipment_records.json", "defaults": DEFAULT_EQUIPMENT, "allowed": set()},
    "supplier": {"file": "supplier_records.json", "defaults": DEFAULT_SUPPLIERS, "allowed": set()},
}
...
def _normalize_parse_text(value) -> str:
    if value is None:
        return ""
    return str(value).translate(_FULLWIDTH_TRANSLATION).strip()
...
def _parse_date(value) -> str:
    ...
    text = _normalize_parse_text(value)
    if not text:
        return ""
    normalized_slash = text.replace(".", "/").replace("-", "/")
    ...
    for fmt in ("%Y/%m/%d", "%Y/%m/%d %H:%M:%S", "%d/%m/%Y", "%d/%m/%Y %H:%M:%S"):
        try:
            return datetime.strptime(normalized_slash, fmt).date().isoformat()
        except ValueError:
            pass
...
def _generate_id(kind: str, record: dict, items: list[dict]) -> str:
    ...
    if kind == "nonconformance":
        ...
        year = (_parse_date(record.get("calibrationDate")) or date.today().isoformat())[:4]
        prefix = f"CAL-{year}-"
        width = 3
    elif kind == "training":
        prefix = "EMP-"
...
def _normalize_nonconformance(record: dict, items: list[dict]) -> dict:
    normalized = {
        ...
        "relatedDocument": str(record.get("relatedDocument") or "").strip(),
        "sourceAuditPlanId": str(record.get("sourceAuditPlanId") or "").strip(),
        "sourceAuditScope": str(record.get("sourceAuditScope") or "").strip(),
...
def _normalize_calibration(record: dict, items: list[dict]) -> dict:
    calibration_date = _parse_date(record.get("calibrationDate") or record.get("date"))
    next_calibration = _parse_date(record.get("nextCalibration"))
    calib_method = str(record.get("calibMethod") or "").strip()
    status = str(record.get("status") or "").strip()
    if not status:
        status = "免校正" if calib_method == "免校" else "合格"
    normalized = {
        "id": str(record.get("id") or "").strip(),
        "instrumentId": str(record.get("instrumentId") or record.get("id") or "").strip(),
        "instrumentName": str(record.get("instrumentName") or record.get("name") or "").strip(),
        ...
        "keeper": str(record.get("keeper") or "").strip(),
        "location": str(record.get("location") or "").strip(),
        "frequencyLabel": str(record.get("frequencyLabel") or "").strip(),
        "note": str(record.get("note") or "").strip(),
        "sourceRecordPath": str(record.get("sourceRecordPath") or "").strip(),
        "sourceInventoryPath": str(record.get("sourceInventoryPath") or "").strip(),
        "sourceReportPath": str(record.get("sourceReportPath") or "").strip(),
    }
    if not normalized["id"]:
        normalized["id"] = _generate_id("calibration", normalized, items)
    return normalized
...
def _normalize_training_entry(employee_id: str, entry: dict, index: int) -> dict:
    date_value = _parse_date(entry.get("date"))
    valid_until = _parse_date(entry.get("validUntil"))
    if not valid_until and date_value and str(entry.get("cert") or "").strip() == "有":
        valid_until = _add_days(date_value, 365)
    return {
        "id": str(entry.get("id") or f"{employee_id}-T{index + 1:02d}").strip(),
        "course": str(entry.get("course") or "").strip(),
        "date": date_value,
        "type": str(entry.get("type") or "內訓").strip(),
        "result": str(entry.get("result") or "待評估").strip(),
        "cert": str(entry.get("cert") or "無").strip(),
        "validUntil": valid_until,
        "hours": _coerce_optional_float(entry.get("hours")),
        "instructor": str(entry.get("instructor") or "").strip(),
        "note": str(entry.get("note") or "").strip(),
    }


def _normalize_training(record: dict, items: list[dict]) -> dict:
    normalized = {
        "id": str(record.get("id") or "").strip(),
        "name": str(record.get("name") or "").strip(),
        "dept": str(record.get("dept") or "").strip(),
        "role": str(record.get("role") or "").strip(),
        "hireDate": _parse_date(record.get("hireDate")),
        "validUntil": _parse_date(record.get("validUntil")),
        "note": str(record.get("note") or "").strip(),
        "source_file": str(record.get("source_file") or "").strip(),
        "trainings": [],
    }
    if not normalized["id"]:
        normalized["id"] = _generate_id("training", normalized, items)
    raw_trainings = record.get("trainings") or []
    normalized["trainings"] = [
        _normalize_training_entry(normalized["id"], entry if isinstance(entry, dict) else {}, idx)
        for idx, entry in enumerate(raw_trainings)
    ]
    if not normalized["validUntil"]:
        valid_dates = sorted(item.get("validUntil") for item in normalized["trainings"] if item.get("validUntil"))
        normalized["validUntil"] = valid_dates[0] if valid_dates else ""
    return normalized


def _normalize_equipment_history(equipment_id: str, entry: dict, index: int) -> dict:
    return {
        "id": str(entry.get("id") or f"{equipment_id}-M{index + 1:03d}").strip(),
...
def _normalize_equipment(record: dict, items: list[dict]) -> dict:
    normalized = {
        "id": str(record.get("id") or "").strip(),
        "name": str(record.get("name") or "").strip(),
        "location": str(record.get("location") or "").strip(),
        "owner": str(record.get("owner") or record.get("keeper") or "").strip(),
        "intervalDays": _coerce_int(record.get("intervalDays"), 0),
        "lastMaintenance": _parse_date(record.get("lastMaintenance")),
        "nextMaintenance": _parse_date(record.get("nextMaintenance")),
        "vendor": str(record.get("vendor") or "").strip(),
        "model": str(record.get("model") or "").strip(),
        "status": str(record.get("status") or "").strip(),
        "note": str(record.get("note") or "").strip(),
        "source_file": str(record.get("source_file") or "").strip(),
        "nextItems": [str(item).strip() for item in (record.get("nextItems") or []) if str(item).strip()],
        "maintenanceHistory": [],
    }
    if not normalized["id"]:
        normalized["id"] = _generate_id("equipment", normalized, items)
    raw_history = record.get("maintenanceHistory") or []
    normalized["maintenanceHistory"] = [
        _normalize_equipment_history(normalized["id"], entry if isinstance(entry, dict) else {}, idx)
        for idx, entry in enumerate(raw_history)
    ]
    if not normalized["lastMaintenance"]:
        history_dates = sorted((item.get("date") for item in normalized["maintenanceHistory"] if item.get("date")), reverse=True)
        normalized["lastMaintenance"] = history_dates[0] if history_dates else ""
    if not normalized["nextMaintenance"]:
        normalized["nextMaintenance"] = _add_days(normalized["lastMaintenance"], normalized["intervalDays"])
    if not normalized["status"]:
        normalized["status"] = "待安排" if not normalized["lastMaintenance"] else "正常"
    return normalized


def _normalize_supplier_eval_history(supplier_id: str, entry: dict, index: int) -> dict:
    score = _coerce_int(entry.get("score"), 0)
    result = str(entry.get("result") or "").strip()
    if not result:
        result = "優良" if score >= 90 else "合格" if score >= 80 else "條件合格" if score >= 70 else "不合格"
    return {
        "id": str(entry.get("id") or f"{supplier_id}-E{index + 1:03d}").strip(),
        "date": _parse_date(entry.get("date")),
        "score": score,
        "result": result,
        "operator": str(entry.get("operator") or "").strip(),
        "remark": str(entry.get("remark") or "").strip(),
        "issues": [str(item).strip() for item in (entry.get("issues") or []) if str(item).strip()],
    }


def _normalize_supplier(record: dict, items: list[dict]) -> dict:
    normalized = {
        "id": str(record.get("id") or "").strip(),
        "name": str(record.get("name") or "").strip(),
        "category": str(record.get("category") or "").strip(),
        "contact": str(record.get("contact") or "").strip(),
        "lastEvalDate": _parse_date(record.get("lastEvalDate")),
        "evalScore": _coerce_int(record.get("evalScore"), 0),
        "evalResult": str(record.get("evalResult") or "").strip(),
        "evalIntervalDays": _coerce_int(record.get("evalIntervalDays"), 365),
    ...
    if not normalized["id"]:
        normalized["id"] = _generate_id("supplier", normalized, items)
    normalized["evalHistory"] = [
        _normalize_supplier_eval_history(normalized["id"], entry if isinstance(entry, dict) else {}, idx)
        for idx, entry in enumerate(record.get("evalHistory") or [])
    ]
    if normalized["evalHistory"]:
        latest_history = sorted(
            [item for item in normalized["evalHistory"] if item.get("date")],
            key=lambda item: item.get("date"),
            reverse=True,
        )
        latest = latest_history[0] if latest_history else normalized["evalHistory"][0]
        if not normalized["lastEvalDate"]:
            normalized["lastEvalDate"] = latest.get("date", "")
        if not normalized["evalScore"]:
            normalized["evalScore"] = latest.get("score", 0)
        if not normalized["evalResult"]:
            normalized["evalResult"] = latest.get("result", "")
        if not normalized["issues"]:
            normalized["issues"] = latest.get("issues", [])
    if not normalized["evalResult"]:
        score = normalized["evalScore"]
        normalized["evalResult"] = "優良" if score >= 90 else "合格" if score >= 80 else "條件合格" if score >= 70 else "不合格"
    return normalized
...
def normalize_record(kind: str, record: dict, items: list[dict]) -> dict:
    if kind == "nonconformance":
        ...
        normalized = _normalize_calibration(record, items)
    elif kind == "training":
        normalized = _normalize_training(record, items)
    elif kind == "equipment":
        normalized = _normalize_equipment(record, items)
    elif kind == "supplier":
        normalized = _normalize_supplier(record, items)
    else:
...
def _extract_labeled_dates(text: str, labels: list[str]) -> list[str]:
    normalized = _normalize_parse_text(text)
    if not normalized or not labels:
        return []
    label_group = "|".join(re.escape(label) for label in labels)
    matches = []
    for match in re.finditer(rf"(?:{label_group})\s*[:：]?\s*([0-9]{{2,4}}[./-][0-9]{{1,2}}[./-][0-9]{{1,2}})", normalized):
...
def _extract_labeled_dates_safe(text: str, labels: list[str]) -> list[str]:
    normalized = _normalize_parse_text(text)
    if not normalized or not labels:
        return []
    label_group = "|".join(re.escape(label) for label in labels)
    pattern = rf"(?:{label_group})\s*[:：]?\s*([0-9]{{2,4}}[./-][0-9]{{1,2}}[./-][0-9]{{1,2}})"
    matches = []
...
def parse_nonconformance_import(path: Path, stored_ref: str) -> dict:
    text = _extract_text(path)
    filename_id = ""
    filename_match = re.search(r"(MR\d{2}-\d{2}\(\d{3}\)A)", path.name, re.IGNORECASE)
    if filename_match:
        filename_id = filename_match.group(1).upper()
    else:
        compact_match = re.search(r"(MR\d{2}-\d{2})(\d{3})A", path.name, re.IGNORECASE)
        if compact_match:
            filename_id = f"{compact_match.group(1).upper()}({compact_match.group(2)})A"
    ...
    draft = {
        **draft,
        "id": _pick_first(
            filename_id,
            draft.get("id"),
            _extract_value(text, ["編號", "不符合單編號", "報告編號", "單號"]),
        ),
        "date": _pick_first(
            draft.get("date"),
            _extract_value(text, ["發現日期", "不符合日期", "日期"]),
            _first_date(text),
        ),
        "dept": _pick_first(
            draft.get("dept"),
            _extract_value(text, ["發現單位", "發現部門", "部門", "單位"]),
        ),
        "description": _pick_first(
            draft.get("description"),
            _extract_value(text, ["不符合事項說明", "問題描述", "不符合事項"]),
        ),
        "rootCause": _pick_first(
            draft.get("rootCause"),
            _extract_value(text, ["原因分析", "根本原因", "Root Cause Analysis"]),
        ),
        "correctiveAction": _pick_first(
            draft.get("correctiveAction"),
            _extract_value(text, ["改善措施", "矯正措施", "處理措施", "預防措施"]),
        ),
        "responsible": _pick_first(
            draft.get("responsible"),
            _extract_value(text, ["責任人", "責任單位", "發現者/稽核員", "發現者", "稽核員"]),
        ),
        "dueDate": _pick_first(
            draft.get("dueDate"),
            _extract_value(text, ["預定完成日期", "預計完成日期", "到期日", "完成日期"]),
        ),
        "closeDate": _pick_first(
            draft.get("closeDate"),
            _extract_value(text, ["結案日期", "實際完成日期", "關閉日期"]),
        ),
        "effectiveness": _pick_first(
            draft.get("effectiveness"),
            _extract_value(text, ["有效性確認", "效果確認", "效果評估"]),
        ),
        "source_file": stored_ref,
    }
    explicit_report_dates = _extract_labeled_dates_safe(text, ["不符合日期", "發現日期"])
    explicit_due_dates = _extract_labeled_dates_safe(text, ["預定完成日期", "預計完成日", "到期日"])
    explicit_close_dates = _extract_labeled_dates_safe(text, ["結案日期", "完成日期"])
    if not str(draft.get("date") or "").strip() and explicit_report_dates:
        draft["date"] = explicit_report_dates[0]
    if not str(draft.get("dueDate") or "").strip() and explicit_due_dates:
        draft["dueDate"] = explicit_due_dates[-1]
    if not str(draft.get("closeDate") or "").strip() and explicit_close_dates:
        draft["closeDate"] = explicit_close_dates[-1]
    if not str(draft.get("type") or "").strip():
        if any(keyword in text for keyword in ("文件", "版次", "程序")):
            draft["type"] = "文件不符"
        elif any(keyword in text for keyword in ("量測", "校正", "儀器")):
            draft["type"] = "量測異常"
        elif any(keyword in text for keyword in ("來料", "供應商")):
            draft["type"] = "來料不合格"
        elif "客訴" in text or "客戶投訴" in text:
            draft["type"] = "客戶投訴"
        else:
            draft["type"] = "製程異常"
    if not str(draft.get("severity") or "").strip():
        if any(keyword in text for keyword in ("重大", "嚴重")):
            draft["severity"] = "重大"
        elif any(keyword in text for keyword in ("中度", "一般")):
            draft["severity"] = "中度"
        else:
            draft["severity"] = "輕微"
    if str(draft.get("closeDate") or "").strip():
        draft["status"] = "已關閉"
    else:
        raw_status = str(draft.get("status") or "").strip()
        if raw_status not in {"待處理", "進行中", "已關閉"}:
            if any(keyword in text for keyword in ("已結案", "已完成", "結案")):
                draft["status"] = "已關閉"
            elif any(keyword in text for keyword in ("改善中", "進行中", "處理中")):
                draft["status"] = "進行中"
            else:
                draft["status"] = "待處理"
    preview = normalize_record("nonconformance", draft, load_records("nonconformance"))
    missing_fields = [field for field in ("date", "dept", "description", "responsible", "rootCause", "correctiveAction", "dueDate") if not preview.get(field)]
...
def _resolve_storage_path(stored_path: str) -> Path | None:
    raw_value = str(stored_path or "").strip()
    if not raw_value:
        return None
    ...
    if candidate_path.is_absolute():
        ...
        cleaned = raw_value.replace("\\", "/").lstrip("/")
        ...
        if cleaned.startswith("uploads/"):
            ...
            if not candidate.exists():
                alt_candidate = (BASE_DIR.parent / cleaned).resolve()
                if alt_candidate.exists():
                    candidate = alt_candidate
                else:
                    filename = Path(cleaned).name
                    exact_matches = list(BASE_DIR.parent.rglob(filename))
                    if exact_matches:
                        candidate = exact_matches[0].resolve()
                    else:
                        def _normalize_attachment_key(name: str) -> str:
                            stem = Path(name).stem.lower()
                            stem = re.sub(r"^\d+\.\d+\s*", "", stem)
                            stem = re.sub(r"^\d+\s*", "", stem)
                            stem = re.sub(r"[\s\-_()（）]+", "", stem)
                            return stem
                        target_key = _normalize_attachment_key(filename)
                        if target_key:
                            fuzzy_matches = []
                            for path in BASE_DIR.parent.rglob("*"):
                                if not path.is_file():
                                    continue
                                if path.suffix.lower() not in {".pdf", ".doc", ".docx", ".xls", ".xlsx", ".csv", ".txt", ".ppt", ".pptx"}:
                                    continue
                                current_key = _normalize_attachment_key(path.name)
                                if current_key == target_key:
                                    fuzzy_matches.append(path.resolve())
                            if fuzzy_matches:
                                fuzzy_matches.sort(key=lambda path: (len(path.name), len(str(path))))
...
def list_auditplan_attachments(record_id: str, items: list[dict] | None = None) -> list[dict]:
    record = next((item for item in (items if items is not None else load_records("auditplan")) if item.get("id") == record_id), None)
    if record is None:
