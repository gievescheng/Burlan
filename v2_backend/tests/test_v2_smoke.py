...
class V2SmokeTest(unittest.TestCase):
    ...
    def test_knowledge_qa(self):
        ingest_response = self.client.post(
            ...
            json={'paths': ['1 文件化資訊管制程序/文件化資訊管制程序2.0.docx'], 'metadata': {}},
        )
        self.assertIn('parser_name', ingest_response.json()['data']['documents'][0])
        self.assertIn('layout_available', ingest_response.json()['data']['documents'][0])
    ...
    def test_document_audit_cache_hit(self):
        audit_payload = {'path': '1 文件化資訊管制程序/文件化資訊管制程序2.0.docx'}
        ...
        self.assertIn('parser_name', first['data'])
        self.assertIn('layout_available', first['data'])
    ...
    def test_cache_status_and_clear(self):
        audit_payload = {'path': '1 文件化資訊管制程序/文件化資訊管制程序2.0.docx'}
        compare_payload = {
            'left_path': '17內部稽核管理程序/內部稽核管理程序.docx',
            'right_path': '18管理審查程序/管理審查程序.docx',
            'use_llm': False,
        }
    ...
    def test_same_family_compare_has_version_conclusion(self):
        response = self.client.post(
            '/api/v2/documents/compare',
            json={
                'left_path': '0  品質手冊/品質手冊(4.0).pdf',
                'right_path': '0  品質手冊/品質手冊(5.0).pdf',
            },
        )
        self.assertEqual(response.status_code, 200)
        payload = response.json()
    ...
    def test_version_candidates(self):
        response = self.client.post(
            ...
            json={'path': '0  品質手冊/品質手冊(5.0).docx', 'limit': 6},
        )
        self.assertEqual(response.status_code, 200)
        payload = response.json()
        self.assertTrue(payload['success'])
        candidate_paths = [item['path'] for item in payload['data']['candidates']]
        self.assertTrue(any('品質手冊(4.0).pdf' in item or '品質手冊(3.0).docx' in item for item in candidate_paths))
    ...
    def test_document_compare_export_docx(self):
        ...
        try:
            ...
            self.assertIn('柏連企業股份有限公司', xml_text)
    ...
    def test_document_audit_export_docx(self):
        response = self.client.post(
            ...
            json={'path': '1 文件化資訊管制程序/文件化資訊管制程序2.0.docx'},
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.headers['content-type'],
